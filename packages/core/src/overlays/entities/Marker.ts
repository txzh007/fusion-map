...
import maplibregl, { type Map as MapLibreMap, type Popup, type Marker as MapLibreMarker } from 'maplibre-gl';
import { BaseOverlay } from './BaseOverlay';
import type { MarkerOptions, IMarker, LngLat, Bounds } from '../../types/overlays';
...
export class Marker extends BaseOverlay implements IMarker {
  ...
  /** 标记内容元素 */
  private markerContentElement: HTMLElement | null = null;
  ...
  addTo(map: MapLibreMap): void {
    ...
    this.marker = new maplibregl.Marker({
      ...
      anchor: (this.options.anchor as any) || 'center'
    });
  ...
  private createMarkerElement(): HTMLElement {
    ...
    if (typeof this.icon === 'string') {
      ...
      content.style.width = this.options.iconSize?.[0] ? `${this.options.iconSize[0]}px` : '30px';
      content.style.height = this.options.iconSize?.[1] ? `${this.options.iconSize[1]}px` : '30px';
      content.style.backgroundImage = `url(${this.icon})`;
      content.style.backgroundSize = 'contain';
      content.style.backgroundRepeat = 'no-repeat';
      content.style.backgroundPosition = 'center';
    } else if (this.icon instanceof HTMLElement) {
      // HTML元素
      content = this.icon.cloneNode(true) as HTMLElement;
    } else {
      // 默认标记
      content = document.createElement('div');
      content.style.width = '30px';
      content.style.height = '30px';
      content.style.backgroundColor = '#3388ff';
    ...
    this.markerContentElement = content;
    this.updateRotation();
    ...
    if (this.options.title) {
      wrapper.title = this.options.title;
    }
  ...
  private updateRotation(): void {
    const element = this.markerContentElement;
    if (!element) {
      return;
    }
  ...
  private createPopup(): void {
    ...
    if (typeof this.popupContent === 'string') {
      popup.setHTML(this.popupContent);
    } else {
      popup.setDOMContent(this.popupContent);
    }
  ...
  protected onOptionsChanged(changedProps: string[]): void {
    const marker = this.marker;
    const options = this.options as MarkerOptions;
    if (!marker) {
      return;
    }
    ...
    changedProps.forEach((prop) => {
      switch (prop) {
        ...
        case 'offset':
          marker.setOffset((options.offset as [number, number]) || [0, 0]);
          break;
